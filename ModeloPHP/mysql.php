...
$DB_USER="u961308898_indoor";
$DB_PASS="Y4Wl+G/l";
$DB_NAME="u961308898_indoor";
    ...
    if($conn->connect_error){                                                           //checks connection
      ...
      if($result === false){
        ...
        if (strlen(stristr($query,"SELECT"))>0) {                                       //tests if it's a SELECT statement
          ...
          $csv = rtrim($csv, ",")."\n";
          // echo json_encode($csv); //envio de dados de cabeçalho da tabela com saída JSON
          $csv = '';

          $result->data_seek(0);
          while($row = $result->fetch_all(MYSQLI_ASSOC)){
            echo json_encode($row); //envio de dados da tabela com saída JSON
          }
        } else {
